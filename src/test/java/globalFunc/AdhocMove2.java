...
public class AdhocMove2 extends UnitTestClassBase {
	...
	public static void adhocMove(DataTable usercredentials)
		...
		try {
			...
			Thread.sleep(4000);
				...
				String errorser= Desktop.describe(Window.class, new WindowDescription.Builder().ownedWindow(false)
						.childWindow(false).windowClassRegExp("PuTTY").windowTitleRegExp(" PuTTY").build()).getVisibleText();
				System.out.println("error-"+errorser);
				
				if(errorser.contains("Error")) {
			...
			String error1= Desktop.describe(Window.class, new WindowDescription.Builder().ownedWindow(false)
					.childWindow(false).windowClassRegExp("PuTTY").windowTitleRegExp(" PuTTY").build()).getVisibleText();
			...
			String error2= Desktop.describe(Window.class, new WindowDescription.Builder().ownedWindow(false)
					.childWindow(false).windowClassRegExp("PuTTY").windowTitleRegExp(" PuTTY").build()).getVisibleText();
			System.out.println("error-"+error2);
			
			if(error2.contains("Error")) {
				System.out.println("AdhocMove failed");
				try {
					Runtime rt = Runtime.getRuntime();
					rt.exec("taskkill /F /IM putty.exe");
